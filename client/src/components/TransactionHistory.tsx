...
export function TransactionHistory() {
    ...
    const fetchHistory = async () => {
        ...
        setLoading(true);
        try {
            const response = await fetch(`https://commandoss-server.duongfinance.com/api/history?count=20`);
    ...
    const searchTransaction = async () => {
        ...
        try {
            const response = await fetch(`https://commandoss-server.duongfinance.com/api/status/${searchHash.trim()}`);
            const data = await response.json();
